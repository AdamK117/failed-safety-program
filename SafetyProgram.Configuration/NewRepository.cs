...
{
    public sealed class NewRepository<TContent> : INewRepository<TContent>
    {
        private readonly ICallbackService<TContent> contentService;
        private readonly IList<TContent> cachedContent = new List<TContent>();

        public NewRepository(ICallbackService<TContent> contentService)
        {
        ...
        public IEnumerable<TContent> GetAll(Action<TContent> callback)
        {
            ...
            {
                var loadedContent = contentService.LoadContent(callback);
