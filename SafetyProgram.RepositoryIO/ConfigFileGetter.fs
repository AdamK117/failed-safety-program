module SafetyProgram.RepositoryIO.ConfigFileGetter
    ...
    let XmlParser<'T when 'T :> IStorable>(rep : IRepositoryInfo) (callback : 'T->unit) (ctor : unit->'T) = 
    ...
    let LoadRepository<'T when 'T :> IStorable> (callback : 'T->unit) (ctor : unit->'T) (repository : IRepositoryInfo) = 
    ...
    let Retrieve<'T when 'T :> IStorable>(callback : 'T->unit) (configFile : IConfiguration) (ctor : unit->'T) = 
        ...
        //Concat the response from each individual command into a total response
        |> Seq.concat

    let RetrieveO<'T when 'T :> IStorable>(configFile : IConfiguration, ctor : unit->'T) = 
